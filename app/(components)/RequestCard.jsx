...
const RequestCard = ({ request_key, sender, memoTrackingNum, details, status, dateSent, session }) => {
    const [loadingDetails, setLoadingDetails] = useState(false);

    const viewRequestDetailsAdmin = `/admin/requests/details?id=${request_key}`;
    const viewRequestDetailsStaff = `/staff/requests/details?id=${request_key}`;
                ...
                <div>
                    <span className='flex flex-col lg:flex-row'>
                        <span className="flex my-2 md:my-0">
                            ...
                            {status === 'no response' && <span className='flex items-center p-1 rounded bg-yellow-300 border border-black mr-2'><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="currentColor" className='mr-1'><path fill="none" d="M0 0h24v24H0z"></path><path d="M12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22ZM11 15V17H13V15H11ZM11 7V13H13V7H11Z"></path></svg>{status}</span>}
                        </span>
                    </span>
                    ...
                    session.user.role === "admin"
                        ? <div className="flex flex-col items-end text-center justify-between mt-2 md:mt-0" onClick={() => setLoadingDetails(!loadingDetails)}>
                            {loadingDetails
                                ...
                                : (
                                    <Link href={viewRequestDetailsAdmin} className="py-1 hover:bg-gray-200 border-black border-b-2 border-dashed cursor-pointer transition-all">View request details</Link>
                                )}
                        </div>
                        : <div className="flex flex-col items-end text-center justify-between mt-2 md:mt-0" onClick={() => setLoadingDetails(!loadingDetails)}>
                            {loadingDetails ? (
                                ...
                                <Link href={viewRequestDetailsStaff} className="py-1 hover:bg-gray-200 border-black border-b-2 border-dashed cursor-pointer transition-all">View request details</Link>
                            )}
                        </div>
                }
            </div>
