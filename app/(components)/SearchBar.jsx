                ...
                <div className="mt-5">
                    {
                        isLoading
                            ?
                            <span className='flex items-center text-center justify-center text-xl'>
                            ...
                            memoResults.length > 0 ? memoResults.map(memoResult => (
                                <MemoCard key={memoResult.id} memo_key={memoResult.id} sender={memoResult.sender} receipient={memoResult.receipient} memoTrackingNum={memoResult.memoTrackingNum} type={memoResult.type} title={memoResult.title} dateSent={formatTimestamp(memoResult.dateSent)} dateConfirmed={formatTimestamp(memoResult.dateConfirmed)} session={session} />
                            )) : (
                                <h4>No memos matched</h4>
                            )
                    }
                </div>
            </div>
