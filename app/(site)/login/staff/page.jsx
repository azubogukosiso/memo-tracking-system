...
const Page = () => {
    const [data, setData] = useState({ email: '', password: '', role: 'staff' });

    const logInStaff = async (e) => {
        e.preventDefault();
        console.log(data);
        const res = await signIn('credentials', { ...data, callbackUrl: 'http://localhost:3000/staff' });
        console.log(res);
    }

    return (
        <form className='w-1/2 mx-auto mt-32' onSubmit={logInStaff}>
            <h1 className='mb-10'>Sign In - Staff</h1>
