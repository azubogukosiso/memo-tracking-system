...
const fetchMemoDetails = async (id) => {
    ...
    try {
        const res = await fetch(`${process.env.URL_ORIGIN}/api/Memos/getMemos?id=${id}`, { cache: "no-store" });
        return res.json();
    } catch (error) {
...
const page = async ({ params }) => {
    ...
    console.log("here we are: ", message.memoTransferHistory);
    return (
        <div>
            <Header />
...

                {
                    message.memoTransferHistory.map((transfer, index) => (
                        <div key={index} className={`mb-3 p-3 rounded ${transfer._id === id ? 'bg-gray-300 border border-black' : ''}`}>
                            <p className='text-base'><span className='font-bold'>Sender:</span> {transfer.sender}</p>

                            <p className='text-base'><span className='font-bold'>Receipient:</span> {transfer.receipient}</p>
